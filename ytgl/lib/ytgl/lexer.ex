defmodule Ytgl.Lexer do
  alias Ytgl.Token

  @doc """
  The `tokenize/1` function is the entry point for the lexer. It takes an input string and returns a list of tokens.

  ## Usage

      iex> Ytgl.Lexer.tokenize("int a = 5;")
      [%Ytgl.Token{type: :int, literal: "int"}, %Ytgl.Token{type: :ident, literal: "a"}, ...]
  """
  ...
  defp read_two_char_operator(chars, tokens) do
    ...
    token =
      case literal do
        ...
        "&&" -> Token.new(type: :logical_and, literal: literal)
        "||" -> Token.new(type: :logical_or, literal: literal)
      end

    tokenize(rest, [token | tokens])
  end

  @spec read_string(nonempty_maybe_improper_list(), any()) :: list()
